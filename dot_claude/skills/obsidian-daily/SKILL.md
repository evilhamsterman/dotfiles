...
1. Find or create the Daily folder: `mcp__claude_ai_Google_Drive__search_files`
2. Check if today's note exists: search for the filename
3. Read if it exists: `mcp__claude_ai_Google_Drive__read_file_content`
4. Create or update: `mcp__claude_ai_Google_Drive__create_file` — **always specify `mimeType: text/plain`** so the result is a plain Markdown file, not a Google Doc
